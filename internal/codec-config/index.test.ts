...
const promise = createFixtureTests(async (fixture, t) => {
	...
	for (const file of fixture.files.values()) {
		...
		t.namedSnapshot(`parse ${ext}`, val.consumer.asUnknown(), undefined, {filename: snapshotFile});
		t.namedSnapshot(`stringify ${ext}`, stringifyConfig(val), undefined, {filename: snapshotFile});
	}
});
